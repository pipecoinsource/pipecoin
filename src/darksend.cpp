...
{
    if(fLiteMode) return; // ignore all Pipe related functionality
...
/*
    ...
    allow endless transaction that would bloat Pipe and make it unusable. To
...
bool CDarksendPool::MakeCollateralAmounts(const CompactTallyItem& tallyItem)
{
    ...
    if(!fSuccess) {
        ...
        LogPrintf("CDarksendPool::MakeCollateralAmounts -- ONLY_NONDENOMINATED_NOT100000IFMN Error: %s\n", strFail);
        CCoinControl *coinControlNull = NULL;
        fSuccess = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
                nFeeRet, nChangePosRet, strFail, coinControlNull, true, ONLY_NOT100000IFMN);
        if(!fSuccess) {
            LogPrintf("CDarksendPool::MakeCollateralAmounts -- ONLY_NOT100000IFMN Error: %s\n", strFail);
...
{
    if(fLiteMode) return; // disable all Pipe specific functionality
