...
const char * const BITCOIN_CONF_FILENAME = "pipe.conf";
const char * const BITCOIN_PID_FILENAME = "piped.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "pipe" is a composite category enabling all Pipe-related debug output
            if(ptrCategory->count(string("pipe"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PipeCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PipeCore
    // Mac: ~/Library/Application Support/PipeCore
    // Unix: ~/.pipecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PipeCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PipeCore";
#else
    // Unix
    return pathRet / ".pipecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty pipe.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override pipe.conf
